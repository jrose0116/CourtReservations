...
router.route("/:courtId/reserve").get(async (req, res) => {
	...
	{
		const strError = "This error occurred in the /:courtId/reserve route because a court with the supplied id does not exist. Click a link on the court by id page to avoid this issue.";
...
router.route("/:courtId/reserve").post(async (req, res) => {
	...
	try {
		...
		{
			if (req.body.capacity.charCodeAt(i) < 48 || req.body.capacity.charCodeAt(i) > 57)
			{
