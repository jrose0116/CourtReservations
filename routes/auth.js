...
.post(async (req, res) => {
    ...
    try {
      ...
      req.session.user = {id: user._id.toString(), firstName: user.firstName, lastName: user.lastName, emailAddress: user.emailAddress, isOwner: user.owner};
      ...
      return res.status(400).render('../views/login', {message: e, error: true});
    }
  });
